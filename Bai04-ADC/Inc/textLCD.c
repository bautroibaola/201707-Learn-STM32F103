...
void lcd_byte(char dulieu)
{
	lcd_nible(dulieu); 				// Truyen len 4 bit cao cua bien byte
	lcd_nible(dulieu << 4); 		// Dich 4 bit thap cua bien byte len cao va truyen
	HAL_Delay(1);
}
...
void lcd_gotoxy(char x, char y)
{
	...
	if(y == 0) lcd_cmd(0x80 + x); //y la dong 1, luc dua vao thi dung lcd_gotoxy(0,0)
	if(y == 1) lcd_cmd(0xC0 + x); //y la dong 2, luc dua vao thi dung lcd_gotoxy(0,1)
	
	//Hoac co the viet
//	if(y == 1) lcd_cmd(0x80 + x - 1); //y la dong 1, luc dua vao thi (1,1)
//	if(y == 2) lcd_cmd(0xC0 + x - 1); //y la dong 2, luc dua vao thi (1,2)
...
void lcd_init(void)
{
	...
	//Hung ==> theo datasheet HD44780.pdf trang 46
	HAL_Delay(15);
